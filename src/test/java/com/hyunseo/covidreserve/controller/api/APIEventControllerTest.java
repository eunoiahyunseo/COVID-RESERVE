...
class APIEventControllerTest {
    ...
    void givenParams_whenRequestingEvents_thenReturnsListOfEventsInStandardResponse() throws Exception {
        ...
        mvc.perform(
                ...
                .andExpect(jsonPath("$.data[0].memo").value("마스크를 꼭 착용해주세요"))
                .andExpect(jsonPath("$.success").value(true))
                .andExpect(jsonPath("$.errorCode").value(ErrorCode.OK.getCode()))
                .andExpect(jsonPath("$.message").value(ErrorCode.OK.getMessage()));

        then(eventService).should(times(1)).getEvents(any(),any(),any(),any(),any());
    }
    ...
    void givenEvent_whenCreatingAnEvent_thenReturnsSuccessfulStandardResponse() throws Exception {
        ...
        then(eventService).should(times(1)).createEvent(any());
    }
    ...
    @Test
    void givenEventId_whenRequestingExistEvent_thenReturnsEventInStandardResponse() throws Exception {
        // Given
        Long eventId = 1L;
        ...
        mvc.perform(
                ...
                .andExpect(jsonPath("$.data.eventStartDatetime")
                        .value(LocalDateTime
                                .of(2021, 1, 1, 13, 0, 0)
                                .format(DateTimeFormatter.ISO_LOCAL_DATE_TIME)))
                .andExpect(jsonPath("$.data.eventEndDatetime")
                        .value(LocalDateTime
                                .of(2021, 1, 1, 16, 0, 0)
                                .format(DateTimeFormatter.ISO_LOCAL_DATE_TIME)))
                .andExpect(jsonPath("$.data.currentNumberOfPeople").value(0))
                .andExpect(jsonPath("$.data.capacity").value(24))
                .andExpect(jsonPath("$.data.memo").value("마스크를 꼭 착용해주세요"))
                .andExpect(jsonPath("$.success").value(true))
                .andExpect(jsonPath("$.errorCode").value(ErrorCode.OK.getCode()))
                .andExpect(jsonPath("$.message").value(ErrorCode.OK.getMessage()));

        then(eventService).should(times(1)).getEvent(eventId);
    }

    @DisplayName("[API][GET] 단일 이벤트 조회 - 이벤트 없는 경우, 빈 표준 API 출력")
    @Test
    void givenEventId_whenRequestingNoneExistEvent_thenReturnsEmptyStandardResponse() throws Exception {
        // Given
        Long eventId = 2L;
    ...
    void givenEvent_whenModifyingAnEvent_thenReturnsSuccessfulStandardResponse() throws Exception {
        ...
        mvc.perform(
                ...
                .andExpect(jsonPath("$.errorCode").value(ErrorCode.OK.getCode()))
                .andExpect(jsonPath("$.message").value(ErrorCode.OK.getMessage()));
        then(eventService).should(times(1)).modifyEvent(eq(eventId), any());
    }
    ...
    void givenEvent_whenDeletingAnEvent_thenReturnsSuccessfulStandardResponse() throws Exception {
        ...
        mvc.perform(delete("/api/events/" + eventId))
                ...
                .andExpect(jsonPath("$.errorCode").value(ErrorCode.OK.getCode()))
                .andExpect(jsonPath("$.message").value(ErrorCode.OK.getMessage()));
        then(eventService).should(times(1)).removeEvent(eq(eventId));
    }
